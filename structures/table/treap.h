...
namespace structures
{
	...
	inline void Treap<K, T>::insert(const K& key, const T& data)
	{
		...
		if (tryToInsertNode(treeNode))
		{
			...
			while (parentPriority != INT_MAX && parentPriority > treapItem->getPriority())
			{
	...
	inline T Treap<K, T>::remove(const K& key)
	{
		...
		if (found)
		{
			...
			while (leftSonPriority != INT_MAX || rightSonPriority != INT_MAX)
			{
				BinarySearchTree<K, T>::BSTTreeNode* rotateSon = nullptr;

				if (leftSonPriority != INT_MAX && rightSonPriority != INT_MAX)
				{
					rotateSon = leftSonPriority < rightSonPriority ? nodeToRemove->getLeftSon() : nodeToRemove->getRightSon();
				}
				else
				{
					rotateSon = leftSonPriority != INT_MAX ? nodeToRemove->getLeftSon() : nodeToRemove->getRightSon();
				}
